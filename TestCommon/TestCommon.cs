...
namespace TestCommon
{
    public static class TestTools
    {
        ...
        public static string Process(string inStr, Func<long, long, long[][], string[], long[]> processor)
        {
            long[][] data;
            var lines = inStr.Split(NewLineChars, StringSplitOptions.RemoveEmptyEntries);
            long nodeCount = long.Parse(lines.First().Split()[0]);
            long eventCount = long.Parse(lines.First().Split()[1]);
            data = ReadTree(lines.Skip(1).Take((int)nodeCount));
            string[] events = lines.Skip(1 + (int)nodeCount).ToArray();
            return string.Join(" ", processor(nodeCount, eventCount, data, events));
        }
        public static string Process(string inStr, Func<long, long, long, long, long[][], long> processor)
        {
            long[][] edges;
            var lines = inStr.Split(NewLineChars, StringSplitOptions.RemoveEmptyEntries);
            long nodeCount = long.Parse(lines.First().Split()[0]);
            long edgeCount = long.Parse(lines.First().Split()[1]);
            long lib = long.Parse(lines.First().Split()[2]);
            long road = long.Parse(lines.First().Split()[3]);
            edges = ReadTree(lines.Skip(1));
            return string.Join(" ", processor(nodeCount, edgeCount, lib, road, edges));
        }
        public static string Process(string inStr,
